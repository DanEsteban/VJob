...
@endphp
    ...
    <input id="order_id" type="text" value="{{$product->id}}" hidden>
    <form action="/inventories/{{$product->id}}"  enctype="multipart/form-data" method="POST">
        @csrf
        @method('PUT')
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="input-group">
                            ...
                            <input name="select_type" type="text" value="{{$product->id_type}}" hidden>
                            <select name="select_type" id="select_type" class="form-select form-select-sm">
                                @foreach ($items_type as $type)
                                    <option value="{{$type->id}}" <?php echo ($type->id ==  $product->id_type) ? ' selected="selected"' : '';?>>{{$type->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-5">
                        <div class="form-check">
                            <input name="cs_inactive" class="form-check-input" type="checkbox" value="{{$product->is_active}}" id="flexCheckDefault" <?php if($product->is_active == 0) echo  'checked'?>>
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="input-group">
                            <label for="select_family" class="col-sm-4 col-form-label form-control-sm" align="left">Group:</label>
                            <select name="select_family" id="select_family" onchange="newGroup();" class="form-select form-select-sm" aria-label=".form-select-sm">
                                <option value=""></option>
                                <option value="0">------------(New)------------</option>
                                @foreach ($groups as $group)
                                    <option value="{{$group->id}}" <?php echo ($group->id ==  $product->id_group) ? ' selected="selected"' : '';?>>{{$group->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="input-group">
                            ...
                            <select name="select_process" id="select_process" class="form-select form-select-sm" aria-label=".form-select-sm">
                                ...
                                @foreach ($process as $pr)
                                    <option value="{{$pr->id}}" <?php echo ($pr->id ==  $product->id_process) ? ' selected="selected"' : '';?>>{{$pr->description}}</option>
                                @endforeach                             
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-5">
                        <div class="input-group">
                            ...
                            <input autocomplete="off" id="item_name" name="item_name" type="text" class="form-control form-control-sm mt-3" value="{{$product->item_name}}" tabindex="1" required/>
                        </div>
                    </div>
                    <div id="div_numpart" class="col-md-5" hidden>
                        <div class="input-group">
                            ...
                            <input autocomplete="off" id="item_part" name="item_part" type="text" class="form-control form-control-sm mt-3" value="{{$product->part_number}}" tabindex="2"/>
                        </div>
                    </div>
                </div>
                <!--- Unidad de Medida --->
                ...
                <div id="div_unit" class="row">
                    <div class="col-md-5">
                        <div class="input-group">
                            <label for="item_unity" class="col-sm-4 col-form-label form-control-sm" align="left">Unit of Measure:</label>
                            <select name="item_unity" id="item_unity" onchange="newUnit();" class="form-select form-select-sm" aria-label=".form-select-sm" tabindex="3">
                                <option value="" selected disabled>Choose an option</option>
                                <option value="0">------------(New)------------</option>
                                @foreach ($units as $unit)
                                    <option value="{{$unit->id}}" <?php echo ($unit->id ==  $product->id_unit_measure) ? ' selected="selected"' : '';?>>{{$unit->description}}-{{$unit->abbreviation}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div id="div_description_p" class="col-md-5">
                        <div class="input-group">
                            ...
                            <textarea name="item_description_p" id="item_description_p" class="form-control form-control-sm" cols="30" rows="3" style="resize: none; width: 272px" tabindex="4">{{$product->purchase_description}}</textarea>
                        </div>
                    </div>
                    <div id="div_description_s" class="col-md-5">
                        <div class="input-group">
                            ...
                            <textarea name="item_description_s" id="item_description_s" class="form-control form-control-sm" cols="30" rows="3" style="resize: none; width: 272px" tabindex="5" required>{{$product->sales_description}}</textarea>
                        </div>
                    </div>
                </div>
                <!--- Subir Imagenes --->
                                ...
                                <tbody>
                                    @foreach ($images as $image)
                                        <tr>
                                            ...
                                            <td><button type="button" onclick="deleteImage({{$image->id}}, this);" class="btn btn-sm btn-danger"><i class="fa-solid fa-trash-can"></i></button></td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                                    ...
                                    <table class="table table-sm table-hover">
                                        ...
                                        <tbody id="tb_items">
                                            @foreach ($items_production as $itm)
                                                @php
                                                    $code = Products::where('id', $itm->id_item)->value('item_name');
                                                    $description = Products::where('id', $itm->id_item)->value('sales_description');
                                                    $qty = AssamblyItems::where('id_item_main', $product->id)->where('id_item',  $itm->id_item)->value('qty');
                                                @endphp
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input onchange="changeItem(this, {{json_encode($items)}});" type="text" name="cod_production[]" class="form-control form-control-sm" list="itemsList" value="{{$code}}">
                                                    ...
                                                    <td><input type="text" id="description_production" class="form-control form-control-sm" value="{{$description}}" readonly></td>
                                                    <td><input type="text" id="qty_production" name="qty_production[]" class="form-control form-control-sm" value="{{$qty}}"></td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                ...
                <div class="row">
                     <div class="">
                        <div class="input-group">
                            ...
                            <textarea name="item_notes" id="item_notes" class="form-control form-control-sm" cols="30" rows="3" style="resize: none; width: 715px" tabindex="7">{{$product->notes}}</textarea>
                        </div>
                    </div>
                </div>
                <hr>
                ...
                <div class="row">
                    <div id="div_costo" class="col-md-5" hidden>
                        <div class="input-group">
                            ...
                            <input autocomplete="off" id="item_cost" name="item_cost" type="text" class="form-control form-control-sm" value="{{$product->cost}}" tabindex="8"/>
                        </div>
                    </div>
                    <br>
                    <div id="div_precio" class="col-md-5">
                        <div class="input-group">
                            ...
                            <input autocomplete="off" id="item_price" name="item_price" type="text" class="form-control form-control-sm" value="{{$product->price}}" tabindex="9"/>
                        </div>
                    </div>
                </div>
                <!--- Margen de utilidad --->
                ...
                <div id="div_maxmin" class="row">
                    <div  class="col-md-5">
                        <div class="input-group">
                            ...
                            <input autocomplete="off" id="item_max" name="item_max" type="text" class="form-control form-control-sm" value="{{$product->max_order}}" tabindex="11"/>
                        </div>
                    </div>
                    <br>
                    <div class="col-md-5">
                        <div class="input-group">
                            ...
                            <input autocomplete="off" id="item_min" name="item_min" type="text" class="form-control form-control-sm" value="{{$product->min_order}}" tabindex="12"/>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-sm">
                            ...
                            <tbody id="tb_body">
                                ...
                                @foreach ($codebars as $code)
                                    <tr>
                                        <td><input name="code_item[]" type="text" class="form-control form-control-sm" value="{{$code->code}}"></td>
                                        <td>
                                            @php
                                                $vendor = Vendors::where('id', $code->id_vendor)->value('name');
                                            @endphp
                                            <input name="code_vendor[]" type="text" class="form-control form-control-sm" value="{{$vendor}}" list="vendorList">
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-footer">
                        <button type="button" onclick="saveUnit();" class="btn btn-outline-primary" data-bs-dismiss="modal">Save</button>
                        <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">Cancelar</button>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $.ajax({
                ...
                success: function (data) {
                    data.forEach(element => {
                        colors.push(element['id_color']);
                    });

                    $('#select_color').val(colors);
        ...
        $.ajax({
                ...
                success: function (data) {
                    data.forEach(element => {
                        sizes.push(element['id_size']);
                    });

                     $('#select_size').val(sizes);
                     $('#select_size').trigger('change'); 
                }
        });
    });

    function deleteImage(id, params) {
        ...
        Swal.fire({
            ...
            denyButtonText: `Cancelar`,
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: "/operations/item/image/delete/"+id,
                        data:{},
                        error: function (xhr, status, error) {
                            console.log(xhr.responseText);
                        },
